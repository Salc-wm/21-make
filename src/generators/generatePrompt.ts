...
import {
  appTypes, appPageTypeOptions, designStyles, colorSchemes, darkModeOptions,
  borderRadiusOptions, densityOptions, fontStyles, fontSizes,
...
export function generatePrompt(config: PromptConfig): string {
  ...
  if (config.projectName || config.projectDescription) {
    ...
    const projectFields = [
      ...
      builder.addField(strings.type, config.appType, appTypes),
      builder.addField(strings.structure, config.appPageType, appPageTypeOptions) +
  ...
  const colorInfo = colorSchemes.find(c => c.value === config.colorScheme);
  const colorPalette = builder.addField(lang === 'pt' ? 'Paleta de Cores' : 'Color Palette', config.colorScheme, colorSchemes) +
  ...
  builder.addSection(`## 🎨 ${strings.visualStyle}`, [
    builder.addField(strings.designStyle, config.designStyle, designStyles),
    colorPalette,
    builder.addField(lang === 'pt' ? 'Modo' : 'Mode', config.darkMode, darkModeOptions),
    builder.addField('Border Radius', config.borderRadius, borderRadiusOptions),
    builder.addField(strings.density || (lang === 'pt' ? 'Densidade' : 'Density'), config.density, densityOptions)
  ]);

  const fontInfo = fontStyles.find(f => f.value === config.fontStyle);
  builder.addSection(`## 🔤 ${strings.typography}`, [
    builder.addField(lang === 'pt' ? 'Família' : 'Font Family', config.fontStyle, fontStyles) + (fontInfo?.example ? ` (${lang === 'pt' ? 'ex' : 'e.g.'}: ${fontInfo.example})` : ''),
    builder.addField(lang === 'pt' ? 'Tamanho Base' : 'Base Font Size', config.fontSize, fontSizes),
  ...
  const layoutFields = [
    builder.addField('Layout', config.layoutType, layoutTypes),
    builder.addField(lang === 'pt' ? 'Navegação' : 'Navigation', config.navigationStyle, navigationStyles),
  ...
  if (config.components.length > 0) {
    builder.addSection(`## 🧩 ${strings.components}`, [
      ...
      builder.addList(config.components.map(c => componentOptionsList.find(o => o.value === c)?.label ?? c)),
      '',
  ...
  if (config.pages.length > 0 || config.customPages.trim()) {
    ...
    const allPages: { key: string; label: string; isCustom?: boolean }[] = [
      ...config.pages.map(p => ({ key: p, label: pageOptionsList.find(o => o.value === p)?.label ?? p })),
  ...
  builder.addSection(`## ✨ ${strings.interactions}`, [
    builder.addField(lang === 'pt' ? 'Animações' : 'Animations', config.animations, animationOptions),
  ...
  builder.addSection(`## 💬 ${strings.toneLang}`, [
    builder.addField(lang === 'pt' ? 'Idioma da UI' : 'UI Language', config.language, languageOptions),
    builder.addField(lang === 'pt' ? 'Tom' : 'Tone', config.tone, toneOptions),
